...
CSM_LIB_DECL hsm_buffer hsm_buffer_alloc(struct hsm_params*);

/** Frees the buffer structure */
CSM_LIB_DECL void hsm_buffer_free(hsm_buffer);

/** Adds a point to the Hough Transform */
CSM_LIB_DECL void hsm_compute_ht_point(hsm_buffer, double x, double y, double weight);


CSM_LIB_DECL void hsm_match(struct hsm_params*p, hsm_buffer b1, hsm_buffer b2);

/** Private interface */

/** This adds a base displacement to the point added by hsm_compute_ht_point */
CSM_LIB_DECL void hsm_compute_ht_base(hsm_buffer, const double base_pose[3]);

/** Computes the spectrum in the buffer */
CSM_LIB_DECL void hsm_compute_spectrum(hsm_buffer);	

CSM_LIB_DECL void hsm_compute_spectrum_norm(hsm_buffer b);
...
CSM_LIB_DECL void hsm_find_local_maxima_circ(int n, const double*f, int*maxima, int*nmaxima);

/** Returns true if @angle1 (rad) and @angle2 (rad) are closer than @threshold_deg (degrees!) */
CSM_LIB_DECL int hsm_is_angle_between_smaller_than_deg(double angle1, double angle2, double threshold_deg);
...
CSM_LIB_DECL int hsm_rho2index(hsm_buffer b, double rho, int *rho_index, double *alpha);

/** Creates circular cross-correlation in a stupid way */
CSM_LIB_DECL void hsm_circular_cross_corr_stupid(int n, const double *a, const double *b, double*res);

/** Finds the peaks of a circular function @f of length @n. */
CSM_LIB_DECL void hsm_find_peaks_circ(int n, const double*f, double min_angle_deg, int unidir, int max_peaks, 
                         int*peaks, int* npeaks) ;

CSM_LIB_DECL void hsm_find_peaks_linear(int n, const double*f, double min_dist, int max_peaks, 
                           int*peaks, int* npeaks);
CSM_LIB_DECL void hsm_find_local_maxima_linear(int n, const double*f, int*maxima, int*nmaxima);

CSM_LIB_DECL void hsm_linear_cross_corr_stupid(int na, const double *a, int nb, const double *b, double*res, int*lags, int min_lag, int max_lag);

CSM_LIB_DECL void hsm_generate_combinations(int nslots, const int possible_choices[], 
                               int i, int i_choice[]) ;

/* a mod b >= 0 */
CSM_LIB_DECL int pos_mod(int a, int b);

/** Sorts the indexes based on the values */
CSM_LIB_DECL void qsort_descending(int *indexes, size_t nmemb, const double*values);

/* used by qsort_descending */
CSM_LIB_DECL int compare_descending(const void *index_pt1, const void *index_pt2);
